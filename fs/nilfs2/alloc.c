		...
		void *data)
{
	...
	for (group = 0; group < ngroups;) {
		...
		for (i = 0; i < n; i++, desc++, group++) {
			m = entries_per_group - nilfs_palloc_group_desc_nfrees(inode, group, desc);
			if (m > 0) {
				ret = nilfs_palloc_get_bitmap_block(
				...
				for (j = 0; j < m; ++j, ++pos) {
					...
					if (pos < entries_per_group) {
						...
						if (blkoff != nilfs_palloc_entry_blkoff(inode, req.pr_entry_nr)) {
							if (blkoff)
								brelse(req.pr_entry_bh);
							ret = nilfs_palloc_get_entry_block(inode, req.pr_entry_nr,
										 0, &req.pr_entry_bh);
							if (ret < 0)
								goto out_entry;
